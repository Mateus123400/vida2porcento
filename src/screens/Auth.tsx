...
import { Mail, Lock, LogIn, UserPlus, AlertCircle, KeyRound, Send } from 'lucide-react';
import { useUser } from '../context/UserContext';
import clsx from 'clsx';

export const Auth: React.FC = () => {
    const { isPasswordRecovery, setIsPasswordRecovery } = useUser();
    const [view, setView] = useState<'login' | 'register' | 'forgot_password' | 'update_password'>(
        isPasswordRecovery ? 'update_password' : 'login'
    );
        ...
        <div className="min-h-screen bg-black text-white flex flex-col justify-center items-center p-4 relative overflow-hidden">
            ...
            >
                <div className="text-center mb-8">
                    ...
                    <h1 className="text-2xl font-bold tracking-tight">Vida 2%</h1>
                    <p className="text-white/50 text-sm mt-2">
                        {view === 'login' && 'Bem-vindo de volta, guerreiro.'}
                        {view === 'register' && 'Inicie sua jornada hoje.'}
                        {view === 'forgot_password' && 'Recupere seu acesso.'}
                        {view === 'update_password' && 'Crie uma nova senha.'}
                    </p>
                </div>
                ...
                <form onSubmit={handleAuth} className="space-y-4">
                    ...
                    {view === 'update_password' && (
                        <div className="space-y-1">
                            <label className="text-xs font-medium text-white/70 ml-1">Confirmar Senha</label>
                            <div className="relative">
                    ...
                    >
                        {loading ? (
                            ...
                            <>Atualizar Senha <KeyRound size={18} /></>
                        )}
                    </button>
                    ...
                    {(view === 'login' || view === 'register') && (
                        <>
                            ...
                            <button
                                ...
                                disabled={loading}
                                className="w-full bg-[#1A1A1A] border border-white/10 text-white font-medium py-3 rounded-xl flex justify-center items-center gap-3 hover:bg-white/5 transition-all disabled:opacity-70 disabled:cursor-not-allowed active:scale-[0.98]"
                            >
                ...
                <div className="mt-8 text-center flex flex-col items-center gap-3">
                    {(view === 'login' || view === 'register') ? (
                        ...
                        >
                            {view === 'login' ? 'Não tem uma conta? Crie aqui.' : 'Já tem uma conta? Faça login.'}
                        </button>
                    ) : (
                        <button
                            onClick={() => {
